...
public class InformationEstimator implements InformationEstimatorInterface {
    ...
    private double cal_estimation(int n) {
        if (n < 0 || n >= myTarget.length)
            return -1;
        if (estimate_result[n] >= 0)
        ...
        if (n == 0)
            ...
            for (int i = 0; i < n; i++) {
                if (result > cal_estimation(i) + iq(i + 1, n + 1))
                    result = cal_estimation(i) + iq(i + 1, n + 1);
            }
    ...
    public double estimation() {
        ...
        if (myTarget.length == 0 || myTarget == null)
            return 0.0;
        // It returns Double.MAX_VALUE when the true value is infinite, or SPACE is not
        // set.
